...
ClientManager.prototype.addSocket = function (socket) {
  ...
  socket.on('join-token-channel', function(message) {
    thisClientManager.joinTokenChannel(socket, message);
  });
 ...
 */
ClientManager.prototype.joinTokenChannel = function (socket, message) {
  var channel = message.channel;
  var token = message.contentToken;

  if (this.settings.debug) {
    console.log('Joining socket to token channel ' + channel + ' using token ' + token);
  }
  ...
  if (this.tokenChannels[channel].tokens[token]) {
    this.tokenChannels[channel].sockets[socket.id] = this.tokenChannels[channel].tokens[token];
    delete this.tokenChannels[channel].tokens[token];

    var notificationMessage = {
      callback: 'clientJoinedTokenChannel',
      data: this.tokenChannels[channel].sockets[socket.id]
    };
    this.publishMessageToContentChannel(channel, notificationMessage);
  }
  else {
    if (this.settings.debug) {
      console.log('Invalid token used to join channel.');
    }
  }
};
