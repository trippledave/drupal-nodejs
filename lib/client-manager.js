...
clientManager.authenticateClientCallback = function (error, response, body) {
  ...
  if (authData.nodejsValidAuthToken) {
    ...
    clientManager.setupClientConnection(authData.clientId, authData, authData.contentTokens);
    clientManager.authenticatedClients[authData.authToken] = authData;
  }
  else {
    console.log('Invalid login for uid "', authData.uid, '"');
    delete clientManager.authenticatedClients[authData.authToken];
  }
};
...
clientManager.cleanupSocket = function (socket) {
  ...
  if (uid != 0) {
    ...
    this.presenceTimeoutIds[uid] = setTimeout(function () {
      clientManager.checkOnlineStatus();
    }, 2000, uid);
  }
    ...
    if (this.tokenChannels[tokenChannel].sockets[socket.id]) {
      ...
      if (this.tokenChannels[tokenChannel].sockets[socket.id].notifyOnDisconnect) {
        ...
        this.contentChannelTimeoutIds[tokenChannel + '_' + uid] = setTimeout(function () {
          clientManager.checkTokenChannelStatus();
        }, 2000, tokenChannel, socket);
      }
